...
static const char PROGMEM INDEX_HTML[] = R"rawliteral(
  ...
  <select id="lang">
    ...
    <option value='tha' selected>Thai</option>
...
// If you still want to execute the code when the button is clicked, you can leave the click event handler intact
    ...
    function TextRecognition() {
      ...
      Tesseract.recognize(
        ...
        firebase.database().ref('data/').update({Text:text});
        if (text=="a"||text=="A") {
   ...
   setInterval(TextRecognition, 10000);
  </script>
  </body>
  </html> 
)rawliteral";
